...
static JSValue parse_args(JSContext *ctx, LoadData *data, bool has_field, int argc, JSValueConst *argv, LoadFunc func) {
    ...
    if (has_field) {
        if (argc > 2) {
            if (qjs_bad_int_arg(ctx, argv[2], 2)) {
                return JS_EXCEPTION;
            }
    ...
    if (argc > var_arg_index) {
        if (qjs_bad_int_arg(ctx, argv[var_arg_index], var_arg_index)) {
            return JS_EXCEPTION;
        }
    ...
    if (argc > (var_arg_index + 1)) {
        if (qjs_bad_int_arg(ctx, argv[var_arg_index + 1], var_arg_index + 1)) {
            return JS_EXCEPTION;
        }
...
static JSValue syscall_exec_cell(JSContext *ctx, JSValueConst this_value, int argc, JSValueConst *argv) {
    ...
    CHECK2(!qjs_bad_int_arg(ctx, argv[3], 3), ERROR_TEMP);
    err = JS_ToUint32(ctx, &length, argv[3]);
    CHECK(err);

    for (int i = argv_offset; i < argc; i++) {
        CHECK2(!qjs_bad_str_arg(ctx, argv[i], i), ERROR_TEMP);
...
static const JSCFunctionListEntry js_ckb_funcs[] = {
    ...
    JS_PROP_INT32_DEF("INDEX_OUT_OF_BOUND", CKB_INDEX_OUT_OF_BOUND, JS_PROP_ENUMERABLE),
    JS_PROP_INT32_DEF("ITEM_MISSING", CKB_ITEM_MISSING, JS_PROP_ENUMERABLE),
    JS_PROP_INT32_DEF("LENGTH_NOT_ENOUGH", CKB_LENGTH_NOT_ENOUGH, JS_PROP_ENUMERABLE),
    JS_PROP_INT32_DEF("INVALID_DATA", CKB_INVALID_DATA, JS_PROP_ENUMERABLE),
    JS_PROP_INT32_DEF("WAIT_FAILURE", CKB_WAIT_FAILURE, JS_PROP_ENUMERABLE),
    JS_PROP_INT32_DEF("INVALID_FD", CKB_INVALID_FD, JS_PROP_ENUMERABLE),
    JS_PROP_INT32_DEF("OTHER_END_CLOSED", CKB_OTHER_END_CLOSED, JS_PROP_ENUMERABLE),
    JS_PROP_INT32_DEF("MAX_VMS_SPAWNED", CKB_MAX_VMS_SPAWNED, JS_PROP_ENUMERABLE),
    JS_PROP_INT32_DEF("MAX_FDS_CREATED", CKB_MAX_FDS_CREATED, JS_PROP_ENUMERABLE),
};
