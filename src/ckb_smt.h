...
int smt_state_insert(smt_state_t *state, const uint8_t *key, const uint8_t *value) {
...
void smt_state_normalize(smt_state_t *state) {
    ...
    while (next < (int32_t)state->len) {
        ...
        while (next < (int32_t)state->len &&
               memcmp(state->pairs[item_index].key, state->pairs[next].key, SMT_KEY_BYTES) == 0) {
            next++;
        }
        if (item_index != sorted) {
            _smt_fast_memcpy(state->pairs[sorted].key, state->pairs[item_index].key, SMT_KEY_BYTES);
            _smt_fast_memcpy(state->pairs[sorted].value, state->pairs[item_index].value, SMT_VALUE_BYTES);
        }
        sorted++;
    }
    state->len = sorted;
...
int _smt_merge_value_is_zero(const _smt_merge_value_t *v) { return v->t == _SMT_MERGE_VALUE_ZERO; }

const uint8_t _SMT_MERGE_NORMAL = 1;
const uint8_t _SMT_MERGE_ZEROS = 2;

/* Hash base node into a H256 */
void _smt_hash_base_node(uint8_t base_height, const uint8_t *base_key, const uint8_t *base_value,
...
void _smt_merge_with_zero(uint8_t height, const uint8_t *node_key, const _smt_merge_value_t *v, int set_bit,
...
void _smt_merge(uint8_t height, const uint8_t *node_key, const _smt_merge_value_t *lhs, const _smt_merge_value_t *rhs,
                _smt_merge_value_t *out) {
...
const _smt_merge_value_t SMT_ZERO = {.t = _SMT_MERGE_VALUE_ZERO, .value = {0}};
 ...
 */
int smt_calculate_root(uint8_t *buffer, const smt_state_t *pairs, const uint8_t *proof, uint32_t proof_length) {
    ...
    while (proof_index < proof_length) {
        switch (proof[proof_index++]) {
            case 0x4C: {
                ...
                _smt_fast_memcpy(stack_keys[stack_top], pairs->pairs[leave_index].key, SMT_KEY_BYTES);
...
int smt_verify(const uint8_t *hash, const smt_state_t *state, const uint8_t *proof, uint32_t proof_length) {
    uint8_t buffer[32];
